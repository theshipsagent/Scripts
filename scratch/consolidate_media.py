...
def setup_logging(log_dir):
    log_file = os.path.join(log_dir, f"media_mover_{datetime.now().strftime('%Y%m%d_%H%M%S')}.log")
...
def move_media():
    ...
    # Create folders
    consolidated_dir = os.path.join(source_dir, "Consolidated_Media")
    ...
    pre_counts = {'total_files': 0, 'media_files': 0}
    for root_dir, _, files in os.walk(source_dir):
        pre_counts['total_files'] += len(files)
        pre_counts['media_files'] += sum(1 for f in files if f.lower().endswith(media_extensions))
    logging.info(f"Pre-analysis: Total files: {pre_counts['total_files']}, Media files: {pre_counts['media_files']}")

    # Load checkpoint for resume
    checkpoint_file = os.path.join(logs_dir, "mover_checkpoint.txt")
    processed_files = set()
    ...
    for root_dir, _, files in os.walk(source_dir):
        for file in files:
            if file.lower().endswith(media_extensions):
                media_files.append(os.path.join(root_dir, file))
    ...
    with tqdm(total=len(media_files), desc="Moving Media Files") as pbar:
    ...
    print("Move process complete. Check logs for details.")
